        ...
        @foreach ($categories as $category)
            <div class="p-4 transition-shadow duration-300 rounded-lg avoid-break" wire:key="{{ 'category-listing-'.$category->id }}">
                <a href="/listing-page?category_id={{ $category->id }}" wire:navigate>
                    <h3 class="text-xl font-bold text-[#2171a7] hover:text-blue-800 underline">{{ $category->name }}</h3>
                </a>
                <ul class="text-blue-500 mt-2 space-y-2 ">
                    @foreach($category->subCategories as $sub_category)
                        <li wire:key="{{ 'sub-categ-listing-'.$sub_category->id }}">
                            <a href="{{ "listing-page?sc_names[0]=$category->id-$sub_category->name" }}" class="text-[#2171a7] text-[18px] hover:text-blue-700" wire:navigate>
                                {{ $sub_category->name }}
                            </a>
                        </li>
                    @endforeach
